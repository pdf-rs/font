...
impl<'a> Cff<'a> {
    fn parse_font<O: Outline>(&self, idx: u32) -> CffFont<O> {
        ...
        let sids: Cow<[SID]> = match charset_offset {
            ...
            offset => {
                let charset = charset(self.data.get(offset ..).unwrap(), num_glyphs).get();
        ...
        */
        ...
        // build glyphs
        let glyphs = char_strings.iter().enumerate().map(|(id, data)| {
